...
# n repetitions. The same routine can be applied to the extreme case or the increased sample
# size example (alternative AMSS specification). Set simulation_X_extreme scripts to T. For 
...
for (i in 1:n) {
  ...
  # simulation_1_cardinality (S) ----
  if (F) {
    source(paste0(getwd(), "/simulation_1_cardinality.R"))
  }
...
if (T) {
  ...
  for (i in 1:n) {
    master.list[[i]] <- readRDS(paste0(getwd(), "/Data/master.list.extreme/out", i, ".rds"))
  }
}
